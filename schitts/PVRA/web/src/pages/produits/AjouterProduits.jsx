...
const AjouterProduit = () => {
  ...
  const handleAjouter = () => {
    ...
    if (!localNomError && !localPrixError && !localConservationError) {
      ...
      axios.post('http://localhost:8080/produits', body).then(() => {
        setSnackbarSeverity('success')
        setShowSnackbar(true)
        setSnackbarMessage('Produit ajouté')
      }).catch(() => {
    ...
    <Box sx={{ height: '90vh' }}>
      ...
      <Box sx={{ paddingY: '3em', width: '60vw' }}>
        <Grid2 container direction='column' spacing={3} sx={{ alignItems: 'center' }}>
          <Box>
            <Typography variant='h4'>Ajouter un produit</Typography>
          </Box>
          <Box>
            <TextField sx={{ width: 250 }} id='nom-produit' label='Nom' variant='outlined' onChange={handleNomChange} error={nomError} />
          </Box>
          <Box>
            <TextField sx={{ width: 250 }} id='prix-produit' label='Prix ($)' variant='outlined' onChange={handlePrixChange} error={prixError} />
          </Box>
          <Box sx={{ width: 250 }}>
          ...
          <Box>
            <TextField sx={{ width: 250 }} id='prix-conservation' label='Durée conservation (jours)' variant='outlined' onChange={handleConservationChange} error={conservationError} />
          </Box>
          <Box>
